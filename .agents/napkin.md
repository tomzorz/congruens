...
| 2026-08-12 | user | Wrote commit-granularity advice into the commit-message skill ("if it doesn't fit in 50 chars you staged two changes, split the commit", "`and` in the subject means two commits"). User disagrees with prescribing how a commit should be sliced | Commit-message guidance covers the MESSAGE only: format, mood, wrapping, what-vs-why. Do not tell the user how to split or stage commits. Anything that doesn't fit the subject goes in the body, not into a second commit |
