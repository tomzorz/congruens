...
Every repository gets the same skeleton: five folders, a root ignore file, a root attributes file with git-lfs wired up, and one ignore file per project inside `sources/`. That skeleton lives at <https://github.com/tomzorz/archetype> and this skill is how you apply it.
...
Five folders in the repository root, each with a job:

| Folder | Holds | Example |
|---|---|---|
| `media` | raw assets the project is built from | the `.psd` for the logo, the `.flp` for the menu music |
| `docs` | documents describing the project | install guide, specs, third party API docs |
| `data` | sample and test files needed to run the project | a small training set, a sample of a weird format the project parses |
| `submodules` | git submodules | |
| `sources` | the actual code, one folder per project | |

The reasoning behind all five is the same: keep the repo root readable, keep raw assets out of the build tree (Unity in particular will generate `.meta` files for anything it can see), and give everyone one obvious place to look. `media` was called `assets` once; it got renamed so it stops colliding with Unity's `Assets`.

Then: the repo-root gitignore and the gitattributes file go in the root, and each project folder under `sources/` gets the ignore file matching its stack.
...
1. **Establish the shape.** Repo name, where it goes on disk, and which stacks live in it. A repo can hold more than one project, so ask for the list, not just one answer. If the answer is obvious from context, say what you assumed and move on.
...
4. **Create the five folders.** `media`, `docs`, `data`, `submodules`, `sources`. Git does not track empty directories, so each one gets a placeholder file until real content lands. Archetype's own `create_folders.sh` names it `remove_when_something_is_here`, which is both the placeholder and the instruction. Delete each one as soon as its folder has something real in it.
...
9. **Report and stop.** List what you created and what is still empty. Do not commit or push unless asked; the commit-message skill applies when you are.
...
Archetype covers the skeleton and nothing else. Do not invent a README, a license, a CI workflow, an editor config, or an agent scaffold as part of initializing a repo. If the user wants those, they will say so, and each one is its own decision.
