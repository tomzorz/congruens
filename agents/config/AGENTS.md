...
- Correcting a recorded fact means replacing it. Never leave `was X`, `formerly`, `overrides` or `superseded` next to the new value, and never keep a ticked or "DONE" entry as a tombstone: a model skimming the line sees two values and may act on the old one, and git or the backup mirror already hold the history. Delete the old text, write the current fact.
...
- Missing a fact while writing prose, a document or an email: use the TK Placeholder skill instead of guessing or leaving a blank. `TK_price`, one double-clickable token, formatted exactly as the finished text will be, and every remaining TK listed when you hand the text over.
